...
class ApiService {
    ...
    constructor(baseUrl: string) {
        if (!baseUrl) {
            throw new Error("A URL base do endpoint deve ser fornecida ao ApiService.");
        }
        this.baseUrl = baseUrl;
    }

    private async request<T>(endpoint: string | undefined, options: RequestInit): Promise<T> {
        ...
        try {
            ...
            throw new Error(`Erro de rede ou resposta inválida ao acessar ${endpoint}: ${error.toString()}`);
        }
    }
